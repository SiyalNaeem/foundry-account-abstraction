...
contract ZkMinimalAccountTest is Test {
    using MessageHashUtils for Transaction;
    ...
    function testZkValidationTransaction() public {
        // Arrange
        address dest = address(usdc);
        uint256 value = 0;
        bytes memory functionData = abi.encodeWithSelector(
    ...
    function _signTransaction(
        ...
        bytes32 usignedTransactionhash = MemoryTransactionHelper.encodeHash(
            transaction
        );
        // bytes32 digest = usignedTransactionhash.toEthSignedMessageHash();

        uint256 ANVIL_DEFAULT_KEY = 0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80;

        (uint8 v, bytes32 r, bytes32 s) = vm.sign(
            ANVIL_DEFAULT_KEY,
            usignedTransactionhash
        );
